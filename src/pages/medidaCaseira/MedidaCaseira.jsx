...
import { AdicionarMedidaCaseira } from './AdicionarMedidaCaseira'

const TABLE_HEAD = ["ID", "Quantidade", "Medida Caseira", ""]; 

export const MedidaCaseira = () => {
  const [showAddModal, setShowAddModal] = useState(false);

  const [medidaCaseiras, setmedidaCaseiras] = useState([]);
  ...
  const getMedidaCaseiras = async () => {
    try {
      const response = await api.get("medidas");
      setmedidaCaseiras(response.data);
    } catch (error) {
      console.error('Erro ao obter os Medidas:', error);
    }
  };
  useEffect (() => {
    getMedidaCaseiras();
  ...
  const handleAddModal = async (id) => {
    ...
    if(id) {
      ...
      setMedidaCaseira(response.data);
    } else{
      setMedidaCaseira({});
    }
  }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setmedidaCaseiras((prevData) => prevData.filter((medidaCaseira) => medidaCaseira.id !== id));
  ...
  return (
    <>
      <Card >
        ...
        <AdicionarMedidaCaseira
          showAddModal={showAddModal}
          handleAddModal={handleAddModal}
          medidaCaseiras={medidaCaseiras}
          setmedidaCaseiras={setmedidaCaseiras}
          setShowAddModal={setShowAddModal}
          medidaCaseira = {medidaCaseira}
          setMedidaCaseira = {setMedidaCaseira}
        />
      </Card>
